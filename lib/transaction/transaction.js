...
// max amount of ratoshis in circulation
...
// Value used for fee estimation (ratoshis per kilobyte)
...
Transaction.prototype.invalidRatoshis = function() {
  var invalid = false;
  for (var i = 0; i < this.outputs.length; i++) {
    if (this.outputs[i].invalidRatoshis()) {
      invalid = true;
    }
  }
  return invalid;
...
Transaction.prototype.getSerializationError = function(opts) {
  ...
  if (this.invalidRatoshis()) {
    return new errors.Transaction.InvalidRatoshis();
  }
...
Transaction.prototype._hasDustOutputs = function(opts) {
  ...
  for (index in this.outputs) {
    ...
    if (output.ratoshis < Transaction.DUST_AMOUNT && !output.script.isDataOut()) {
 ...
 * transaction.from({'txId': '0000...', outputIndex: 0, ratoshis: 1000, script: 'OP_DUP ...'});
 *
 * // From a multisig P2SH output
 * transaction.from({'txId': '0000...', inputIndex: 0, ratoshis: 1000, script: '... OP_HASH'},
 *                  ['03000...', '02000...'], 2);
 * ```
 *
 ...
 * set, two additional parameters, `outputScript` and `ratoshis` can be provided.
 *
 * @param {Input} input
 * @param {String|Script} outputScript
 * @param {number} ratoshis
 * @return Transaction this, for chaining
 */
Transaction.prototype.addInput = function(input, outputScript, ratoshis) {
  $.checkArgumentType(input, Input, 'input');
  if (!input.output && (_.isUndefined(outputScript) || _.isUndefined(ratoshis))) {
    throw new errors.Transaction.NeedMoreInfo('Need information about the UTXO script and ratoshis');
  }
  if (!input.output && outputScript && !_.isUndefined(ratoshis)) {
    outputScript = outputScript instanceof Script ? outputScript : new Script(outputScript);
    $.checkArgumentType(ratoshis, 'number', 'ratoshis');
 ...
 * be reset).
 *
 * @param {number} amount ratoshis to be sent
 * @return {Transaction} this, for chaining
 */
 ...
 * be reset).
 *
 * @param {number} amount ratoshis per KB to be sent
 * @return {Transaction} this, for chaining
 */
 ...
 * @param {number} amount in ratoshis
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.to = function(address, amount) {
  if (_.isArray(address)) {
    ...
    _.each(address, function(to) {
      self.to(to.address, to.ratoshis);
    });
    return this;
  }
...
/**
 * Calculates or gets the total output amount in ratoshis
 *
...
Transaction.prototype._getOutputAmount = function() {
  if (_.isUndefined(this._outputAmount)) {
    ...
    _.each(this.outputs, function(output) {
      self._outputAmount += output.ratoshis;
    });
  }
...
/**
 * Calculates or gets the total input amount in ratoshis
 *
...
Transaction.prototype._getInputAmount = function() {
  if (_.isUndefined(this._inputAmount)) {
    ...
    _.each(this.inputs, function(input) {
      ...
      self._inputAmount += input.output.ratoshis;
    });
  }
 ...
 *
 * @return {Number} fee of this transaction in ratoshis
 */
...
Transaction.prototype.sort = function() {
  ...
  this.sortOutputs(function(outputs) {
    ...
    copy.sort(function(first, second) {
      return first.ratoshis - second.ratoshis
...
Transaction.prototype.verify = function() {
  ...
  for (var i = 0; i < this.outputs.length; i++) {
    ...
    if (txout.invalidRatoshis()) {
      return 'transaction txout ' + i + ' ratoshis is invalid';
    }
    if (txout._ratoshisBN.gt(new BN(Transaction.MAX_MONEY, 10))) {
      return 'transaction txout ' + i + ' greater than MAX_MONEY';
    }
    valueoutbn = valueoutbn.add(txout._ratoshisBN);
